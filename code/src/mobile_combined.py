...
class Combined_ViT_MobileNet(nn.Module):
    def __init__(self, dropout_rate, n_ori_outputs, n_pos_outputs):
        ...
        # Load pretrained ViT
        self.vit = timm.create_model('vit_tiny_patch16_224', pretrained=False, num_classes=0)  # No final layer
    ...
    def forward(self, x):
        ...
        vit_features = self.vit(self.resize(x))
...
def copy_state_dict_comb(src_state_dict, dst_state_dict):
...
def import_combined_model(dropout_rate, ori_type, n_ori_bins, pretrained=True):
    ...
    if pretrained:
        ...
        model.vit.load_state_dict(copy_state_dict_comb(vit_model.state_dict(), model.vit.state_dict()))

        # Load pretrained MobileNetV2 weights (already loaded in the constructor)
        mobilenet_model = mobilenet_v2(pretrained=True)
        model.mobilenet.load_state_dict(copy_state_dict_comb(mobilenet_model.state_dict(), model.mobilenet.state_dict()))
